...
describe('Ukrainian', () => {
  ...
  it('should report missing translations', () => {
    throws(() => Ukrainian.translate('missing'), (err) => {
      eq(err.code, MissingTranslationPullRequest.code);
      eq(err.message, 'Ukrainian is missing a translation for the "missing" keyword - Please submit a pull request via https://github.com/acuminous/twiki-gherkish-feature-parser/pulls');
      return true;
    });
  });
});
