export default class BaseEvent {
  ...
  checkHandler(state) {
    if (!this._hasEventHandler(state)) throw new Error(`${state.name} is missing an ${this.constructor._handlerName} event handler`);
  }

  checkNoHandler(state) {
    if (this._hasEventHandler(state)) throw new Error(`${state.name} has a redundant ${this.constructor._handlerName} event handler`);
  }
