...
export class UnexpectedEventError extends TwikiError {
  ...
  constructor(state, event, context) {
    const message = `I did not expect ${event.description} at ${context.source.appellative}\nInstead, I expected one of:\n${state.describeExpectedEvents()}`;
...
export class MissingEventHandlerBug extends TwikiBug {
  ...
  constructor(state, context) {
    const message = `${state.name} has no event handler for '${context.source.line}' at ${context.source.appellative}`;
...
export class UnexpectedNumberOfColumnsError extends TwikiError {
  ...
  constructor(source, session, numberOfColumns) {
    const message = `Expected ${session.numberOfExamples} columns but found ${numberOfColumns} at ${source.appellative}`;
...
export class UnexpectedNumberOfExamplesError extends TwikiError {
  ...
  constructor(source, session, numberOfExamples) {
    const message = `Expected ${session.numberOfExamples} examples but found ${numberOfExamples} at ${source.appellative}`;
