...
export class UnexpectedNumberOfExamples extends TwikiError {
  ...
  constructor(source, session, examples) {
    const message = `Expected ${session.numberOfExamples} examples but found ${examples.length} at ${session.metadata?.source?.uri}:${source.number}`;
...
export class ConflictingStateAlias extends TwikiBug {
  static code = `${pkg.name}-00303`;

  constructor(state) {
    const message = `${state.name} is already aliased`;
    super(message, ConflictingStateAlias.code);
  }
}
