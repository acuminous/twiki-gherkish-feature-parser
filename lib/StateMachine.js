...
export default class StateMachine {
  ...
  interpret(source) {
    debug(`[${this.state.name}] Interpretting "${source.line}"`);
  ...
  checkpoint() {
    debug(`[${this.state.name}] Checkpointing ${this._currentState.name}`);
    this._checkpoints.push(this._currentState);
    return this;
  }

  dispatch(event, context) {
    debug(`[${this.state.name}] Dispatching ${event?.name} to ${this._currentState.name} with data: %o`, context.data);
  ...
  unwind() {
    ...
    debug(`[${this.state.name}] Unwinding from ${this._currentState.name} to ${checkpoint.name}`);
