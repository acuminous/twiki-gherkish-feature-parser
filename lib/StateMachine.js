...
export default class StateMachine {
  ...
  get _currentState() {
    ...
    if (!state) throw new Error('The state machine stack is empty. This is a bug, please report it to via the twiki-gherkish-feature-parser repository.');
    return state;
  }

  get _currentCheckpoint() {
    const checkpoint = this._checkpoints[this._checkpoints.length - 1];
    if (!checkpoint) throw new Error('The state machine has been asked to unwind but no checkpoint exists. This is a bug, please report it to via the twiki-gherkish-feature-parser repository.');
    return checkpoint;
  }
