...
export default class StateMachine {
  ...
  constructor(props = {}, useStates2 = false) {
    ...
    const { featureBuilder = new FeatureBuilder(), session = new Session() } = props;
  ...
  interpret(source) {
    this._getCurrentState().interpret(source, this._session);
  }

  checkpoint() {
    const state = this._getCurrentState();
    debug(`Checkpointing ${state.name}`);
    this._checkpoints.push(state);
    return this;
  }

  dispatch(event, context) {
    event.dispatch(this._getCurrentState(), this._session, context);
  }
  ...
  toPreviousCheckpoint() {
    ...
    const state = this._checkpoints.pop();
    debug(`Unwound to state: ${state?.name}`);
    return this;
  }
  ...
  _defineStateTransistions() {
    Object.values(this._useStates2 ? States2 : States).forEach((StateClass) => {
      ...
      this[transitionName] = () => {
        ...
        debug(`Transitioning to ${state.name}`);
