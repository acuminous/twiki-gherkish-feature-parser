...
export default class Session {
  ...
  isProcessingImplicitDocstring() {
    const result = Boolean(this.docstring && !this.docstring.token);
    debug(`Checking if session is currently processing an implicit docstring: ${result}`);
    return result;
  }

  isProcessingMatchingDocstring(token) {
    const result = this.docstring?.token === token;
  ...
  beginExplicitDocstring(token, indentation) {
    this.docstring = { type: 'explicit', token, indentation };
  }
