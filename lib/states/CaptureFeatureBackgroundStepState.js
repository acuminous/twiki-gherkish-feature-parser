...
export default class CaptureFeatureBackgroundStepState extends BaseState {
  ...
  onExplicitDocstringStart() {
    this._machine.alias(EndFeatureBackgroundDocstringState).toBeginExplicitDocstringState();
  }

  onImplicitDocstringStart(session, event, context) {
    this._machine.alias(EndFeatureBackgroundDocstringState).toCaptureImplicitDocstringState().interpret(context.source);
  }
