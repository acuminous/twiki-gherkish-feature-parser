...
export default class BaseState {
  ...
  #events;
  #expectedEvents;
  #debug = debug(`twiki:gherkish-feature-parser:states:${this.#name}`);
  ...
  static aliasTransitionHandler(target) {
    ...
    const underlyingHandler = target[this._handlerName];
  ...
  onUnexpectedEvent(session, event, context) {
    throw new UnexpectedEventError(this, event, context);
  }

  onMissingEventHandler(session, event, context) {
    throw new MissingEventHandlerBug(this, context);
  }
