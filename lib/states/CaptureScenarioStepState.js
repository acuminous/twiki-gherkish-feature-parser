...
export default class CaptureScenarioStepState extends BaseState {
  ...
  onExplicitDocstringStart() {
    this._machine.alias(EndScenarioDocstringState).toBeginExplicitDocstringState();
  }

  onImplicitDocstringStart(session, event, context) {
    this._machine.alias(EndScenarioDocstringState).toCaptureImplicitDocstringState().interpret(context.source);
  }
