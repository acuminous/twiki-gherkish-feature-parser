...
export default class CaptureScenarioStepState extends BaseState {
  ...
  onExplicitDocstringStart() {
    this._machine.checkpoint().alias(EndScenarioDocstringState).toBeginExplicitDocstringState();
  }

  onImplicitDocstringStart(session, event, context) {
    this._machine.checkpoint().alias(EndScenarioDocstringState).toCaptureImplicitDocstringState().interpret(context.source);
  }
